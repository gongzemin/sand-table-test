<template>
  <div class="leftDraw">
    ...
    <div class="leftMainContent">
      ...
      <Tabs type="card" class="tabcard">
        <TabPane label="" icon="md-analytics">
        ...
        <TabPane label="" icon="ios-browsers">
          <div class="middleCont">
            <div class="listout">
              <div class="listin">
                <!--粒子特效-->
                <Tree @on-check-change="getParticleShow" :data="particleEffect" :render="renderParticleEffect" show-checkbox class="treeListStyle GLB" :class="{'active':ParticleEffectToggleExpand === false}" @on-toggle-expand="getParticleEffectToggleExpand()">
                </Tree>
                <!-- POI标绘 -->
                <Tree @on-check-change="POIShow" :data="POIMarks" :render="renderPOI" show-checkbox class="treeListStyle GLB" :class="{'active':POIToggleExpand === false}" @on-toggle-expand="getPOIToggleExpand()">
                </Tree>
                <!-- 矢量图形 -->
                <Tree @on-check-change="VectorShow" :data="VectorGP" :render="renderVectorGP" show-checkbox class="treeListStyle GLB" :class="{'active':VectorGPToggleExpand === false}" @on-toggle-expand="getVectorGPToggleExpand()">
                </Tree>
                <!--GLB模型-->
                <Tree @on-check-change="GLBShow" :data="GLBModel" :render="renderGLBModel" show-checkbox class="treeListStyle GLB" :class="{'active':GLBToggleExpand === false}" @on-toggle-expand="getGLBToggleExpand()">
                </Tree>
              </div>
            </div>
          </div>
    ...
    <!--修改粒子特效的名称-->
    <Modal class="sandModal" v-model="particleName" draggable scrollable title="粒子名称编辑" @on-ok="ok">
      <div><Input v-model="editParticleName" placeholder="Enter something..." /></div>
    </Modal>
    <!--POI标绘名称修改-->
    <Modal class="sandModal" v-model="POIModal" title="POI标绘编辑" @on-ok="POIok" @on-cancel="POIcancel" draggable>
      <Input v-model="POIName" placeholder="Enter something..." />
    </Modal>
    <!--矢量图形名称修改-->
    <Modal class="sandModal" v-model="VectorModal" title="矢量图形编辑" @on-ok="Vectorok" @on-cancel="Vectorcancel" draggable>
      <Input v-model="VectorName" placeholder="Enter something..." />
    </Modal>
    <!--确认批量删除对话框 -->
     <Modal class="sandModal" v-model="batchDeletionFlag" title="批量删除" @on-ok="batchDeletion"  draggable>
       <div style="color:#ffffff;text-align:center;">确定删除选中的&nbsp;{{batchTitle}}&nbsp;数据吗？</div>
    </Modal>
    <!--单体模型姿态修改-->
    <Modal class="sandModal" v-model="GLBModal" title="单体模型编辑" @on-ok="GLBModalok" @on-cancel="GLBModalcancel" draggable>
      <div class="glbAdjust">
        名称：<Input v-model="glbOptions.name" placeholder="Enter something..." style="width:436px;" @on-change="adjustGLB" />
      </div>
      <div class="glbAdjust">
        经度：<InputNumber style="width:190px;"  class="glbAdjustInput"  :step="0.000001"  v-model="glbOptions.lng" @on-change="adjustGLB"></InputNumber>
         <span style="margin-left:10px;">纬度：</span><InputNumber style="width:190px;"  class="glbAdjustInput"   :step="0.000001"  v-model="glbOptions.lat" @on-change="adjustGLB"></InputNumber>
        </div>
      <div class="glbAdjust">
        高度：<InputNumber style="width:190px;"  class="glbAdjustInput"  :step="0.1"  v-model="glbOptions.height" @on-change="adjustGLB"></InputNumber>
      <span style="margin-left:10px;">旋转角度(四周)：</span><InputNumber  style="width:125px;" class="glbAdjustInput"  :max="180.0" :min="-180.0" :step="0.5"  v-model="glbOptions.heading" @on-change="adjustGLB"></InputNumber>
      </div>
      <br>
...
var Particle = require("../libs/DESource/Particle");
var POI = require("../libs/DESource/POI");
var EditEntity = require("../libs/DESource/EditEntity");
var MilitaryPlotting =null;
...
export default {
  ...
  methods: {
    ...
    getParticle(item) {
      ...
      switch (item.id) {
        ...
        case 2://水
          ...
          Particle.addWaterParticleSystem(window.viewer,options);
          break;
        default:
          break;
      }
    ...
    getParticleShow: function (selectedArr, selectedItem) {
    ...
    // POI标绘的显示与隐藏
    POIShow: function (selectedArr, selectedItem) {
    ...
    // 矢量图形的显示与隐藏
    VectorShow: function (selectedArr, selectedItem) {
    ...
    // 调整模型姿态位置等参数
    adjustGLB(){
      var position = Cesium.Cartesian3.fromDegrees(parseFloat(this.glbOptions.lng)||this.glbOriginalOptions.lon, parseFloat(this.glbOptions.lat)||this.glbOriginalOptions.lat, parseFloat(this.glbOptions.height)||0.0);
			var heading = Cesium.Math.toRadians(parseFloat(this.glbOptions.heading));
			var hpr = new Cesium.HeadingPitchRoll(heading||0.0, 0.0, 0.0);
      var orientation = Cesium.Transforms.headingPitchRollQuaternion(position,hpr);
      var a=window.viewer.entities.getById('GLB_'+this.glbOptions.id);
      if(a&&a._show==true){
    ...
    // GLB单体模型的显示与隐藏
    GLBShow: function (selectedArr, selectedItem) {
    ...
    // glb模型参数修改取消
    GLBModalcancel(){
      var position = Cesium.Cartesian3.fromDegrees(parseFloat(this.glbOriginalOptions.lon), parseFloat(this.glbOriginalOptions.lat), parseFloat(this.glbOriginalOptions.altitude));
			var heading = Cesium.Math.toRadians(parseFloat(this.glbOriginalOptions.heading));
			var hpr = new Cesium.HeadingPitchRoll(heading||0.0, 0.0, 0.0);
      var orientation = Cesium.Transforms.headingPitchRollQuaternion(position,hpr);
      var a=window.viewer.entities.getById('GLB_'+this.glbOptions.id);
      if(a){
    ...
    timestamp() {
      var time = new Date();
      var dd = time.getDate();
      var mm = time.getMonth() + 1; //一月是0，一定要注意
